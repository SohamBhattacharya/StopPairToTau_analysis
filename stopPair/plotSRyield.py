...
inputFiles_data_TTCR = Common.getRootFiles(Details.cutFlowDir_elMu_data, suffix_TTCR, rootFileName)
...
inputFiles_WJetsToLNu_TTCR = Common.getRootFiles(Details.cutFlowDir_WJetsToLNu, suffix_TTCR, rootFileName)
inputFiles_DYJetsToLL_TTCR = Common.getRootFiles(Details.cutFlowDir_DYJetsToLL, suffix_TTCR, rootFileName)
inputFiles_VH_TTCR = Common.getRootFiles(Details.cutFlowDir_VH, suffix_TTCR, rootFileName)
inputFiles_VV_TTCR = Common.getRootFiles(Details.cutFlowDir_VV, suffix_TTCR, rootFileName)
inputFiles_WG_TTCR = Common.getRootFiles(Details.cutFlowDir_WG, suffix_TTCR, rootFileName)
inputFiles_ttbar_TTCR = Common.getRootFiles(Details.cutFlowDir_ttbar, suffix_TTCR, rootFileName)
inputFiles_singleTop_TTCR = Common.getRootFiles(Details.cutFlowDir_singleTop, suffix_TTCR, rootFileName)


lumi_data = 8.347e3
lumi_data_TTCR = 35.9e3
...
details_data_TTCR = "OS_CSVv2Mcut1"
details_mc_TTCR = "OS_CSVv2Mcut1"
...
inputFiles_mc_TTCR = [
    inputFiles_WJetsToLNu_TTCR,
    inputFiles_DYJetsToLL_TTCR,
    inputFiles_VH_TTCR,
    inputFiles_VV_TTCR,
    inputFiles_WG_TTCR,
    inputFiles_ttbar_TTCR,
    inputFiles_singleTop_TTCR,
]
...
for iSR in range(0, nSR) :
    ...
    cutFlowName_TTCR = "reco/" + cutFlowNameBase_TTCR + str(int(iSR+1))
    
    columnLabels = []
    
    # Data
    h1_cutFlow_data = Common.getCombinedCutFlow(
        inputFiles = inputFiles_data,
        cutFlowName = cutFlowName,
        details = "tightTight_OS",
        debug = debug
    )
    
    arr_binContent = Common.histToArray(h1_cutFlow_data, "binContent")
    binContent = arr_binContent[0][-1]
    
    arr_binError = Common.histToArray(h1_cutFlow_data, "binError")
    ...
    ttbar_SF_info = Common.getTTbarSF(
        cutFlowName_TTCR,
        inputFiles_data_TTCR, lumi_data_TTCR, Details.label_elMu_data[0], details_data_TTCR,
        inputFiles_mc_TTCR, crossSections_mc, labels_mc, details_mc_TTCR,
        debug)
    ...
    for iMC in range(0, len(labels_mc)) :
        ...
        if (labels_mc[iMC] == Details.label_ttbar[0]) :
            
            #print ttbar_SF_info[0][0]
            binContent *= ttbar_SF_info[0][0]
            binError = Common.computeError(binContent, binError, ttbar_SF_info[0][0], ttbar_SF_info[0][1], "*")
    ...
    for iSig in range(0, len(labels_stopPair)) :
        ...
        # Fill table
        table_yield_stopPair[:, iSig] = arr_binContent[0]
    ...
    if (printTable) :
        ...
        with numpy.errstate(divide = "ignore", invalid = "ignore") :
            ...
            columnLabels += [Details.label_tauTau_data[0]]
            ...
            os.system("mkdir -p cutFlowTables")
            cutFlowFileName = "cutFlowTables/cutFlow_" + cutFlowNameBase + str(int(iSR+1)) + ".txt"
...
if (printTable) :
    ...
    cutFlowFileName = "cutFlowTables/cutFlow_" + cutFlowNameBase + "all.txt"
    print "Writing:", cutFlowFileName
...
if (plotYield) :
    ...
    canvas.SaveAs("%s/SRyield_%s.pdf" %(plotDir, cutFlowNameBase))
